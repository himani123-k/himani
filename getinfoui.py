            ...
            try:
                ...
                while True:
                    ...
                    if self.gettininfo == a:
                        n = 1
                        stringdisplay = "NMAE :" + s.name + "\n" + "ADDRESS :" + s.address + "\n" + "MOBILE NO :"
                        stringdisplay = stringdisplay + (s.mobile_no) +"\n" + "TOTAL BILL :"
                        string = s.price
                        New_Window = tk.Toplevel()
                        New_Window.geometry("600x400")
                        New_Window.title("Guest Information")
                        scrollbar_v = Scrollbar(New_Window, orient="vertical")
                        scrollbar_v.pack(side=tk.RIGHT, fill=tk.Y)
                        txt = tk.Text(New_Window, wrap="none", width=60, height=30, padx=5, pady=5, spacing1=10,
                                      spacing2=10, spacing3=10)
                        txt["yscrollcommand"] = scrollbar_v
                        scrollbar_v.config(command=txt.yview)
                        txt.insert(tk.INSERT, stringdisplay)
                        txt.insert(tk.INSERT, string)
                        txt.pack()

                        New_Window.mainloop()
                    elif EOFError:
                        if n == 0:
                            warning = messagebox.showinfo("NO GUEST IN THIS ROOM")
